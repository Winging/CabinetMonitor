...
//RESET
#define SET_RESET  LPC_GPIO0->FIOSET |= 0x01<<9       
#define CLR_RESET  LPC_GPIO0->FIOCLR |= 0x01<<9 

//A0---H -> DATA, L -> 指令 
#define SET_A0  LPC_GPIO0->FIOSET |= 0x01<<7         
#define CLR_A0  LPC_GPIO0->FIOCLR |= 0x01<<7


//RW
#define SET_RW LPC_GPIO0->FIOSET |= 0x01<<8    //
#define CLR_RW LPC_GPIO0->FIOCLR |= 0x01<<8    //

//E1
#define SET_E1 LPC_GPIO2->FIOSET |= 0x01<<8        
#define CLR_E1 LPC_GPIO2->FIOCLR |= 0x01<<8

//E2
#define SET_E2 LPC_GPIO2->FIOSET |= 0x01<<9       
#define CLR_E2 LPC_GPIO2->FIOCLR = 0x01<<9



//LEDDISPLY
#define SET_LEDDISPLY  LPC_GPIO0->FIOSET |= 0x01<<6            
#define CLR_LEDDISPLY  LPC_GPIO0->FIOCLR = 0x01<<6  

#define SET_GLCDRW_READ  LPC_GPIO0->FIOSET = 0X01 << 17;//读
#define CLR_GLCDRW_WRITE  LPC_GPIO0->FIOCLR = 0X01 << 17; //写  
...
void INITLCD (void)
{
  LPC_GPIO0->FIOSET = 0X01 << 9;            //REST=1;
  nop();
  LPC_GPIO0->FIOCLR = 0X01 << 9;            //REST=0;
  nop();
  LPC_GPIO0->FIOSET = 0X01 << 9;            //REST=1;
  nop();
  WRCOMMAND1 (0xe2);//复位
...
void key_init(void)
{
	LPC_GPIO1->FIODIR &= ~(0X3F << 23);    //按键端口设为输入
}
uint8_t key_ok()
{
	//P1_27   key_ok
